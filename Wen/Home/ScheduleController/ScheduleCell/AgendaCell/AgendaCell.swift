...
class AgendaCell: UITableViewCell, ScheduleTableDelegate {
    ...
    public var delegate: AgendaCellDelegate! {
        didSet {
    ...
    public static func calculateHeightForEvents(for events: [Event], announcement: String?) -> CGFloat {
        var cellHeight = ScheduleTable.calculateHeightForEvents(for: events) + AnnouncementHeight
